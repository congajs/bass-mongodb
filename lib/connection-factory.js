...
module.exports = {
	...
	factory: function(config, cb){
		...
		var connectionHash = config.host + ':' + config.port + poolName + serverOptions.hash;
		...
		if (!mongoClients[connectionHash]) {
			...
			if (serverOptions.options.poolSize){
				urlOptions['maxPoolSize'] = serverOptions.options.poolSize;
			}

			if (serverOptions.options.readPreference){
				urlOptions['readPreference'] = serverOptions.options.readPreference;
			}

			if (clientOptions.options.w){
				urlOptions['w'] = clientOptions.options.w;
			}
			...
			// Add any options that are not supported by the url
			var dbOptions = {};
			if (clientOptions.options.slaveOk){
				dbOptions.slaveOk = clientOptions.options.slaveOk;
			}

			var mongosOptions = {};
			if (clientOptions.options.ha){
				mongosOptions.ha = clientOptions.options.ha;
			}

			var svrOptions = {};
			if (serverOptions.options['auto-reconnect']){
				svrOptions.socketOptions = {autoReconnect: serverOptions.options['auto-reconnect']};
			}

			_lock.beginAsync(connectionHash);
			MongoClient.connect(url, {db: dbOptions, server: svrOptions, mongos: mongosOptions}, function(err, db){

				mongoClients[connectionHash] = db;
